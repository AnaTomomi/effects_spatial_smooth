...
group_mask='/m/cs/scratch/networks/data/ABIDE_II/Forward/masks/group_roi_mask-Brainnetome_0mm-0-2mm_with_subcortl_and_cerebellum_correctedMNI.mat';
load(group_mask)

%Move some centroids a tiny bit
rois_bra(90).centroidMNI = [44,-14,-26];
rois_bra(96).centroidMNI = [50,-8,-28];
rois_bra(237).centroidMNI = [-4,-10,8];

for i=1:246
    label{i}=rois_bra(i).label;
    centroid(i,1:3)=rois_bra(i).centroidMNI;
end
...
save('/m/cs/scratch/networks/data/ABIDE_II/Forward/masks/group_roi_mask-sphere-0-2mm_with_subcortl_and_cerebellum.mat','rois')