...
folder='/m/cs/scratch/networks/data/ABIDE_II/Forward/Brainnetome_0mm/TCD_II';
d= dir([folder '/*']);
d = d(3:end);
for i=1:(length(d))
...
%This is a mock array, it is not used later. Its purpose is to prevent an
%error from the parhandle_bramila function
for i=1:(length(d))
    subjects_out{i} = sprintf('%s/ROI/%s',folder,d(i).name);
end   
