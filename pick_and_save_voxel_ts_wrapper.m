...
main_folder = '/m/cs/scratch/networks/data/ABIDE_II/Forward/';
main_dir=dir(main_folder);
main_dir=main_dir(3:18);

for smooth=1:size(main_dir,1)
    smooth_folder= [main_folder main_dir(smooth).name];
    smooth_dir=dir(smooth_folder);
    smooth_dir=smooth_dir(3:end);
    for site=1:size(smooth_dir)
        site_folder=[main_folder main_dir(smooth).name '/' smooth_dir(site).name];
        d=dir([site_folder '/*']);
        d=d(3:end);
        for i=1:(length(d))
            subjects{i} = sprintf('%s/%s',site_folder,d(i).name);
        end
        ...
        for i=1:(length(d))
            subjects_out{i} = sprintf('%s/ROIs/%s',site_folder,d(i).name);
            %fprintf(sprintf('%s \n', subjects_out{i}))
        end 
