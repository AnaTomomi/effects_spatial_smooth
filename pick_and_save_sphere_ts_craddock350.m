...
function cfg = pick_and_save_sphere_ts_craddock350(cfg)
...
for i = 1:1:length(roi_indices)
    ...
    if ~ roi_index == 0
        ...
        if ~isempty(roi_map)
            ...
            %store which regions are not used in a log
            fid = fopen(fullfile(cfg.inputfolder, 'RoiLog.txt'), 'a');
            if fid == -1
                error('Cannot open log file.');
            end
            fprintf(fid, 'Warning! %s not defined!', rois_to_use{i});
            fclose(fid);
            
            roi_ts(i, :) = [];
            roi_voxel_ts = [];
            %roi_offset_indices = []; %revised by Onerva
        end
    else
        roi_offset_indices = [];
        roi_voxel_ts = [];
...
if strcmp(cfg.adjacency_rois{1}, 'all')
    roi_voxel_ts_info_name = ['sphere_craddock35007_ts_all_rois_info.mat'];%roi_voxel_ts_info_name = ['roi_voxel_ts_all_rois_info.mat'];
    roi_voxel_ts_name = ['sphere_craddock35007_ts_all_rois.mat'];%roi_voxel_ts_name = ['roi_voxel_ts_all_rois.mat'];
else
    roi_voxel_ts_info_name = ['sphere_craddock35007_ts_voxel_info.mat'];%roi_voxel_ts_info_name = ['roi_voxel_ts_info.mat'];
    roi_voxel_ts_name = ['sphere_craddock35007_ts_voxel.mat'];%roi_voxel_ts_name = ['roi_voxel_ts.mat'];
end
