...
savepath= sprintf('/m/cs/scratch/networks/data/ABIDE_II/Analysis/NBS/%s/NBS_Brainnetome_%smm_%s_Sphere',pipeline,smoothing,test);
figure_path=sprintf('/m/cs/scratch/networks/trianaa1/Paper1/Figures/%s/NBS_Brainnetome_%smm_%s_Sphere',pipeline,smoothing,test);
...
excel_file='/m/cs/scratch/networks/trianaa1/Paper1/Significant_Nets_Sphere.xlsx';
...
group1= dir(folder1);
group1= group1(3:end); group1= group1(4:4:end);
group2= dir(folder2);
group2= group2(3:end); group2= group2(4:4:end);
...
UI.node_coor.ui=centroids; %coordinates in MNI space                         
...
%raw(init_row,:)={pipeline,smoothing,i_lab,j_lab,num2str(stat)};
%init_row=init_row+1;
end
raw=cell2table(raw);
writetable(raw,excel_file,'WriteVariableNames',false)
else
raw(init_row,:)={pipeline,smoothing,'NaN','NaN','NaN'};
end