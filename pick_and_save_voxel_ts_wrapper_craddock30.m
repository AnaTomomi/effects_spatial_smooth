...
% 
% for i=1:(length(d))
%     if ~isfile(sprintf('%s/%s/roi_craddock30_ts_all_rois_info.mat',d(i).folder,d(i).name))
...
for i=1:(length(d))
    if ~isfile(sprintf('%s/%s/roi_craddock3007_ts_all_rois_info.mat',d(i).folder,d(i).name))
        subjects{i} = sprintf('%s/%s',d(i).folder,d(i).name);
    end
end
...
cfg.roi_mask_names = {'/m/cs/scratch/networks/data/UCLA_openneuro/masks/FD07/group_roi_mask-craddock30-0-2mm_with_subcortl_and_cerebellum.mat'};
...
for i = 1:length(subjects)
    ...
    cfg.vol = data.img;
    cfg = pick_and_save_voxel_ts_craddock30(cfg);
end
